...
pub async fn info() -> Result<()> {
    ...
    let wallet: WalletInfo = rpc.call("getwalletinfo", json!([])).await?;

    let balances: Balances = rpc.call("getbalances", json!([])).await?;
...
pub async fn balance() -> Result<()> {
    ...
    let balance: f64 = rpc.call("getbalance", json!([])).await?;
